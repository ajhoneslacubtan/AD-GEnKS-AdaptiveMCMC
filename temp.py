...
# Load the extra data to get the true advection
...
# Extract posterior samples of nu from idata1
...
# Time axis (excluding the last time step)
time = np.arange(nu_mean.shape[1] - 1)

# Create figure with two subplots for v_x and v_y
fig, axs = plt.subplots(2, 1, figsize=(8, 6), constrained_layout=True)
fig.suptitle("Analysis 1: Window=3", fontsize=14)

# Plot v_x
axs[0].fill_between(time, nu_lower[0, :-1], nu_upper[0, :-1],
                    color='blue', alpha=0.2, label="95% Credible Interval")
axs[0].plot(time, nu_mean[0, :-1], 'b--', linewidth=2, label="Posterior Mean")
axs[0].plot(time, true_nu[0, :-1], 'k-', linewidth=2, label="True $v_x$")
axs[0].set_ylabel("$v_x$")
axs[0].set_title("$v_x$ over Time")
axs[0].legend(loc="best")

# Plot v_y
axs[1].fill_between(time, nu_lower[1, :-1], nu_upper[1, :-1],
                    color='green', alpha=0.2, label="95% Credible Interval")
axs[1].plot(time, nu_mean[1, :-1], 'g--', linewidth=2, label="Posterior Mean")
axs[1].plot(time, true_nu[1, :-1], 'k-', linewidth=2, label="True $v_y$")
